...
func try_unlock() -> void:
	...
	if token_address.length()==0:
			token_balance = SolanaService.get_sol_balance(account_address.get_value())
	else:
		token_balance = SolanaService.get_token_balance(account_address.get_value(),token_address)
